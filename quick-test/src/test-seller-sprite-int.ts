...
async function test() {
  ...
  try {
    ...
    // 访问网站
    log('🌐 正在打开 https://www.sellersprite.com/...', 'info');
    ...
    // 检查登录状态
    const isLoggedIn = await checkLoginStatus(page, 'https://www.sellersprite.com/');
    if (isLoggedIn) {
    ...
    // 截图
    await screenshot(page, 'seller-sprite-int');
    log('📸 截图已保存: output/seller-sprite-int.png', 'info');

    // 成功
    log('✅ Seller Sprite国际 测试完成！', 'success');
