...
async function test() {
  ...
  try {
    ...
    // 访问网站
    log('🌐 正在打开 https://www.sellersprite.com/cn/...', 'info');
    await page.goto('https://www.sellersprite.com/cn/', {
      timeout: 30000,
    ...
    // 检查登录状态
    const isLoggedIn = await checkLoginStatus(page, 'https://www.sellersprite.com/cn/');
    if (isLoggedIn) {
    ...
    // 截图
    await screenshot(page, 'seller-sprite-cn');
    log('📸 截图已保存: output/seller-sprite-cn.png', 'info');

    // 成功
    log('✅ Seller Sprite中文 测试完成！', 'success');
