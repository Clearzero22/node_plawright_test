...
async function test() {
  ...
  try {
    ...
    // 访问网站
    log('🌐 正在打开 https://chatgpt.com/...', 'info');
    ...
    // 检查登录状态
    const isLoggedIn = await checkLoginStatus(page, 'https://chatgpt.com/');
    if (isLoggedIn) {
    ...
    // 截图
    await screenshot(page, 'chatgpt');
    log('📸 截图已保存: output/chatgpt.png', 'info');

    // 成功
    log('✅ ChatGPT 测试完成！', 'success');
