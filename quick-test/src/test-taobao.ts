...
async function test() {
  ...
  try {
    ...
    // 访问网站
    log('🌐 正在打开 https://www.taobao.com/...', 'info');
    ...
    // 检查登录状态
    const isLoggedIn = await checkLoginStatus(page, 'https://www.taobao.com/');
    if (isLoggedIn) {
    ...
    // 截图
    await screenshot(page, 'taobao');
    log('📸 截图已保存: output/taobao.png', 'info');

    // 成功
    log('✅ Taobao 测试完成！', 'success');
