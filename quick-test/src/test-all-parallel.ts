...
async function testSite(site: Site): Promise<TestResult> {
  ...
  try {
    ...
    log(`✅ ${site.displayName} 测试完成 (${duration}ms)`, 'success');
...
async function testAllParallel() {
  ...
  log(`成功: ${successCount}/${sites.length}`, 'success');
  ...
  results.forEach((result, index) => {
    ...
    log(`${status} ${result.site.displayName}`, result.success ? 'success' : 'error');
    ...
    log(`   截图: ${result.screenshotPath || '无'}`, 'info');
  ...
  log(`\n${exitCode === 0 ? '✅ 所有测试通过！' : '❌ 部分测试失败！'}`, exitCode === 0 ? 'success' : 'error');
